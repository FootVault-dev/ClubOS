...
import { Inbox, Globe, Mail, Instagram, Facebook, MessageCircle, X, Phone, Reply, Check, Archive, Users, ClipboardList, CheckCircle2, XCircle, MapPin } from "lucide-react";
      ...
      {view === "board" && (
        regsLoading ? <div className="text-center py-12 text-white/20 text-sm">Loading…</div> : (
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {AGE_GROUPS.map((g) => {
              const inGroup = activeRegs.filter((r) => r.ageGroups.includes(g));
              return (
                    ...
                    <div className="space-y-1.5">
                      {inGroup.map((r) => (
                        <button key={r.id} onClick={() => setSelectedReg(r)} className="w-full text-left rounded-lg border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] transition-colors px-3 py-2" data-testid={`cic-reg-${g}-${r.id}`}>
                          ...
                          <div className="text-[11px] text-white/45 truncate mt-0.5">{regName(r)}{r.club ? "" : ""} · {r.email}</div>
                        </button>
                      ))}
                    </div>
                  )}
      ...
      <div className="bg-[#141511] border border-amber-500/15 rounded-2xl w-full max-w-lg shadow-2xl max-h-[88vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 overflow-y-auto flex-1 space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {reg.sourceUrl && <span className="text-[11px] text-white/30">via {reg.sourceUrl}</span>}
          </div>
        </div>
