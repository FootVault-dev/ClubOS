...
// CIC Youth "Register Your Interest" submissions from cicyouth.com. Self-contained
// copy of the league inbox, scoped to the CIC org via the /api/admin/cic/inbox
// endpoints (leaves the MFL inbox untouched).
type Msg = {
...
export default function CicInbox() {
    ...
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-white/40 mt-1">Team enquiries from cicyouth.com — also emailed to info@cicyouth.com</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-5">
          <div className="flex flex-col items-center justify-center py-16 text-white/20">
            ...
            <p className="text-xs mt-1">"Register Your Interest" submissions from cicyouth.com land here.</p>
          </div>
        </div>
      ) : (
        <div className="space-y-2">
          {shown.map((m) => {
            ...
            return (
              <button key={m.id} onClick={() => setSelected(m)} className="w-full text-left rounded-xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors p-4 flex items-start gap-3" data-testid={`cic-inbox-row-${m.id}`}>
