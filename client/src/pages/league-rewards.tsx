...
import { Trophy, Mail, Phone, X, Crown, Loader2, Gift, ChevronRight, Ticket, Check, Shirt } from "lucide-react";
...
export default function LeagueRewards() {
  const [view, setView] = useState<"builders" | "season">("builders");
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-white">Rewards</h1>
          <p className="text-sm text-white/40 mt-1">{view === "builders" ? "League Builders — referrals, points & account credit" : "Season Ticket — Team XP, loyalty tiers & vouchers"}</p>
        </div>
        <div className="inline-flex rounded-lg border border-white/5 bg-white/[0.02] p-0.5">
          {([["builders", "League Builders"], ["season", "Season Ticket"]] as const).map(([v, label]) => (
      ...
      {view === "builders" ? <BuildersView /> : <SeasonView />}
    </div>
  );
}
