...
import { Inbox, Globe, Mail, Instagram, Facebook, MessageCircle, X, Phone, Reply, Check, Archive, Loader2 } from "lucide-react";
...
export default function LeagueInbox() {
  const [filter, setFilter] = useState<"all" | "new" | "replied" | "archived">("all");
  const [selected, setSelected] = useState<Msg | null>(null);

  const { data: messages = [], isLoading } = useQuery<Msg[]>({
    queryKey: ["/api/admin/league/inbox"],
    queryFn: () => fetch("/api/admin/league/inbox").then((r) => r.json()),
  });
    ...
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-white/40 mt-1">All-in-one customer inbox — website, email, social &amp; live chat in one place</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-5">
          <div className="flex flex-col items-center justify-center py-16 text-white/20">
            ...
            <p className="text-xs mt-1">Website contact-form enquiries land here. Email, Instagram, Facebook &amp; live chat are coming next.</p>
          </div>
        </div>
      ) : (
        <div className="space-y-2">
          {shown.map((m) => {
            ...
            return (
              <button key={m.id} onClick={() => setSelected(m)} className="w-full text-left rounded-xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors p-4 flex items-start gap-3" data-testid={`inbox-row-${m.id}`}>
...
function MessageModal({ msg, onClose }: { msg: Msg; onClose: () => void }) {
  ...
  const setStatus = useMutation({
    mutationFn: (status: string) => apiRequest("POST", `/api/admin/league/inbox/${msg.id}/status`, { status }).then((r) => r.json()),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/admin/league/inbox"] }); },
    onError: (e: any) => toast({ title: "Couldn't update", description: e.message, variant: "destructive" }),
  });
  const replyHref = msg.email
    ? `mailto:${msg.email}?subject=${encodeURIComponent("Re: " + (msg.subject || "your enquiry — Mini Football Leagues"))}`
    : undefined;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-[#0a0e1a] border border-blue-500/15 rounded-2xl w-full max-w-lg shadow-2xl max-h-[88vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 border-t border-white/5 flex flex-wrap items-center gap-2">
          {replyHref && <a href={replyHref} onClick={() => setStatus.mutate("replied")} className="flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 rounded-lg bg-blue-500/90 text-white hover:bg-blue-500"><Reply className="w-3.5 h-3.5" /> Reply via email</a>}
