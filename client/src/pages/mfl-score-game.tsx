// MFL office "Score Game" screen — /admin/mfl-score/:id. Cloned from
// client/src/pages/cic-score-game.tsx.
//
// Session-authed twin of the referee scoring screen
// (client/src/pages/mfl-ref/MflRefGameDetail.tsx), reached from the Game
// Feed (client/src/components/mfl-game-feed.tsx) so Isaac/the MFL
// coordinator can run the same live match timer + score/goals/cards flows
// from the office exactly like a referee does from pitch-side — including
// the own-goal team-flip rule. No MVP / Golden Glove / penalty shootout —
// leagues don't run any of those, and MFL teams have no player rosters, so
// every goal/card is a free-text name against a credited team.
//
// The one real difference from the referee screen: every write here goes
// through the ADMIN session (apiRequest — cookie + X-Workspace-Slug,
// client/src/lib/queryClient.ts), never the referee Bearer-token helpers in
// ./mfl-ref/mfl-ref-api.ts. Types are imported `type`-only from mfl-ref-api
// (erased at compile time — no runtime coupling to the referee token logic)
// since the admin detail endpoint returns the same underlying DB shapes.
//
// NOTE ON ENDPOINTS: the backend contract this app was built against
// explicitly names GET /api/admin/mfl/games/:id/detail and POST
// /api/admin/mfl/games/:id/timer. Score/goals/cards admin write routes
// weren't separately enumerated (league games had no goal/card tracking
// before this build), so this screen mirrors the public referee routes'
// shape 1:1 under the same /api/admin/mfl/games/:id/* namespace — the
// natural admin-authed counterpart to
// /api/public/mfl-referees/games/:id/{goals,cards}. If the server lands on
// different paths, only the mutationFn URLs below need updating.
...
import type {
  ...
  MflRefTeam,
  RefTimerAction,
} from "@/pages/mfl-ref/mfl-ref-api";

const GOLD = "#d1b96e";
const INK = "#000000";

const FONT_STYLE = `
  .mfl-score-display { font-family: 'Anton', 'Inter Tight', 'Inter', sans-serif; }
  .mfl-score-body { font-family: 'Inter Tight', 'Inter', sans-serif; }
`;
...
const scoreGameQueryKey = (id: number) => ["/api/admin/mfl/games", id, "detail"] as const;

// ═══════════════════════════ Page ══════════════════════════════════════════
export default function MflScoreGame() {
  const [, params] = useRoute("/admin/mfl-score/:id");
  const [, navigate] = useLocation();
  const id = params?.id ? parseInt(params.id, 10) : NaN;

  const { data, isLoading, isError, error, isFetching } = useQuery({
    queryKey: Number.isFinite(id) ? scoreGameQueryKey(id) : ["mfl-score-game", "invalid"],
    queryFn: async () => (await apiRequest("GET", `/api/admin/mfl/games/${id}/detail`)).json() as Promise<ScoreGameDetailResponse>,
    enabled: Number.isFinite(id),
  });

  const goBack = () => navigate("/admin/mfl-game-feed");
  ...
  return (
    <div className="mfl-score-body min-h-screen w-full pb-16" style={{ background: INK }}>
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <GoalsCard game={game} goals={goals} homeName={homeName} awayName={awayName} gameId={id} />
        <CardsCard game={game} cards={cards} homeName={homeName} awayName={awayName} gameId={id} />
      </div>
    </div>
  );
}
...
function StatusScreen({ loading, message, onBack }: { loading?: boolean; message?: string; onBack?: () => void }) {
  return (
    <div className="mfl-score-body min-h-screen w-full flex flex-col items-center justify-center px-6 text-center" style={{ background: INK }}>
      <style>{FONT_STYLE}</style>
      {loading ? (
...
function TimerSection({
  ...
  const timerMut = useMutation({
    mutationFn: (action: RefTimerAction) => apiRequest("POST", `/api/admin/mfl/games/${gameId}/timer`, { action }),
      ...
      <div className="flex items-center gap-3 px-3 py-2.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-sm font-bold text-white">
            ...
            <span className="mfl-score-display tabular-nums shrink-0" style={{ color: GOLD }}>
...
function ScoreCard({
  ...
  const patchMut = useMutation({
    mutationFn: (body: Record<string, any>) => apiRequest("PATCH", `/api/admin/mfl/games/${gameId}`, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: key }),
          ...
          return (
            <div key={side} className="text-center">
              ...
              <div className="flex items-center justify-center gap-2">
                ...
                <div className="mfl-score-display text-6xl font-black tabular-nums w-16 text-center text-white">{value ?? 0}</div>
...
function GoalsCard({
  ...
  const addMut = useMutation({
    mutationFn: (body: any) => apiRequest("POST", `/api/admin/mfl/games/${gameId}/goals`, body),
    onSuccess: () => {
  ...
  const delMut = useMutation({
    mutationFn: (goalId: number) => apiRequest("DELETE", `/api/admin/mfl/games/${gameId}/goals/${goalId}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: key }),
  ...
  const submit = () => {
    ...
    // scorer's own team — mirrors the referee screen's submit() exactly.
...
function CardsCard({
  ...
  const addMut = useMutation({
    mutationFn: (body: any) => apiRequest("POST", `/api/admin/mfl/games/${gameId}/cards`, body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: key });
      setName("");
      setMinute("");
  ...
  const delMut = useMutation({
    mutationFn: (cardId: number) => apiRequest("DELETE", `/api/admin/mfl/games/${gameId}/cards/${cardId}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: key }),
