// ─────────────────────────────────────────────────────────────────────────────
// CIC REFEREES — the staff-facing "Referees" admin page, CIC (tournament)
// workspace. Manages referees who self-signed-up to score games on their
// phones (server/cic-referee-routes.ts, a SEPARATE identity from ClubOS staff
// users — see that file's header comment for why).
//
// Two views: APPROVALS (who signed up, approve/decline/suspend them) and
// ASSIGNMENTS (which games each approved referee is assigned to — a soft
// default for their "My games" list; any approved ref can still score any CIC
// game, assignment is accountability not a hard lock).
//
// House style + react-query/apiRequest patterns copied from group-hiring.tsx
// (stat-chip status filters that double as filters, dark-glass Tailwind,
// inline mutations). Status list comes from @shared/referees — never
// redeclared here. Admin routes are org-scoped by the X-Workspace-Slug header,
// which apiRequest / the default queryFn attach automatically from
// localStorage — no explicit header wiring needed here.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Types (mirror server/cic-referee-routes.ts response shapes) ────────────
interface CicReferee {
  id: number;
  fullName: string;
  email: string;
  phone: string;
...
// Mirror shared/schema.ts `tournaments` / `tournamentGames` / `tournamentTeams`
...
// ── Config ───────────────────────────────────────────────────────────────────
const STATUS_META: Record<RefereeStatus, { label: string; color: string }> = {
...
// shifts a bare date by the browser's timezone offset (the exact bug that
// printed the wrong day on the first invoice-pages deploy).
...
// ═════════════════════════════════════════════════════════════════════════════
export default function CicReferees() {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-[13px] text-white/40 mt-1 max-w-xl">
            Approve referee sign-ups and assign them to games for the Christchurch International Cup.
          </p>
        </div>
...
function ApprovalsView() {
  ...
  const queryKey = ["/api/admin/cic-referees"];
  const { data, isLoading } = useQuery<{ referees: CicReferee[] }>({ queryKey });
  const referees = data?.referees ?? [];

  const invalidate = () => queryClient.invalidateQueries({ queryKey });

  const updateMut = useMutation({
    mutationFn: ({ id, status }: { id: number; status: RefereeStatus }) =>
      apiRequest("PATCH", `/api/admin/cic-referees/${id}`, { status }),
    onSuccess: () => invalidate(),
    onError: (e: unknown) => toast({ title: "Couldn't update status", description: apiErrorMessage(e), variant: "destructive" }),
  });

  const deleteMut = useMutation({
    mutationFn: (id: number) => apiRequest("DELETE", `/api/admin/cic-referees/${id}`),
    onSuccess: () => {
      invalidate();
      // Assignments for this referee are cascade-deleted server-side.
      queryClient.invalidateQueries({ queryKey: ["/api/admin/cic-referees/assignments"] });
  ...
  const counts = useMemo(() => {
    ...
    for (const st of REFEREE_STATUSES) c[st] = 0;
...
function RefereeCard({ referee, onSetStatus, onDelete }: {
  referee: CicReferee;
  onSetStatus: (status: RefereeStatus) => void;
  onDelete: () => void;
}) {
...
function AssignmentsView() {
  ...
  const { data: refData } = useQuery<{ referees: CicReferee[] }>({ queryKey: ["/api/admin/cic-referees"] });
  const approvedReferees = (refData?.referees ?? []).filter((r) => r.status === "approved");

  // GET /api/admin/tournament/tournaments?orgId=… takes the org as a query
  // param (not the X-Workspace-Slug header), so it needs its own queryFn.
  const { data: tournaments = [], isLoading: tournamentsLoading } = useQuery<TournamentSummary[]>({
    ...
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/admin/tournament/tournaments?orgId=${orgId}`);
      return res.json();
    },
    enabled: orgId != null,
  });

  const { data: games = [], isLoading: gamesLoading } = useQuery<TournamentGame[]>({
    queryKey: ["/api/admin/tournament/tournaments", selectedTournamentId, "games"],
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/admin/tournament/tournaments/${selectedTournamentId}/games`);
      return res.json();
    },
  ...
  const { data: teams = [] } = useQuery<TournamentTeam[]>({
    queryKey: ["/api/admin/tournament/tournaments", selectedTournamentId, "teams"],
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/admin/tournament/tournaments/${selectedTournamentId}/teams`);
      return res.json();
    },
    enabled: selectedTournamentId != null,
  });
  ...
  const assignmentsQueryKey = ["/api/admin/cic-referees/assignments"];
  const { data: assignData } = useQuery<{ assignments: RefereeAssignment[] }>({ queryKey: assignmentsQueryKey });
  ...
  const invalidateAssignments = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ["/api/admin/cic-referees"] });
  };
  ...
  // DELETE, and express.json() is mounted globally (server/index.ts) so it
  // parses the body on every method, not just POST/PATCH. No query-string
  // fallback needed.
  ...
  return (
    <>
      <div className="grid gap-3 sm:grid-cols-2 mb-4">
        ...
        <div>
          <label className="text-[11px] text-white/40 mb-1 block">Tournament</label>
          <select
            value={selectedTournamentId ?? ""}
            onChange={(e) => setSelectedTournamentId(e.target.value ? Number(e.target.value) : null)}
            data-testid="select-assign-tournament"
            className={inputCls + " cursor-pointer"}
            disabled={tournamentsLoading || !tournaments.length}
          >
          ...
          {!tournamentsLoading && !tournaments.length && (
            <p className="text-[11px] text-white/25 mt-1">No tournaments found for this workspace.</p>
          )}
        </div>
      </div>

      {selectedRefereeId == null || selectedTournamentId == null ? (
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] py-16 text-center">
          <CalendarCheck className="w-8 h-8 text-white/20 mx-auto mb-3" />
          <div className="text-white/50 text-sm">Pick a referee and a tournament to see its games.</div>
        </div>
      ) : gamesLoading ? (
        <div className="text-white/30 text-sm py-16 text-center">Loading games…</div>
      ) : !sortedGames.length ? (
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] py-16 text-center">
          <div className="text-white/50 text-sm">This tournament has no games scheduled yet.</div>
        </div>
      ) : (
        <div className="space-y-1.5">
          ...
          {sortedGames.map((game) => {
            ...
            const home = game.homeTeamId != null ? (teamName(game.homeTeamId) ?? "TBC") : (game.homeTeamPlaceholder || "TBC");
            const away = game.awayTeamId != null ? (teamName(game.awayTeamId) ?? "TBC") : (game.awayTeamPlaceholder || "TBC");
            return (
              <label
                key={game.id}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2.5 flex-wrap text-[11px] text-white/40 mt-0.5">
                    <span className="capitalize">{game.stage}{game.stageDetail ? ` · ${game.stageDetail}` : ""}</span>
                    {game.field && <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{game.field}</span>}
